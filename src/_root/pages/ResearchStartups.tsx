...
const ResearchStartups = () => {
  ...
  const [activeTab, setActiveTab] = useState<"research" | "startups" | "cofounders" | "pitches" | "myapps">(
    "research"
  );
  ...
  return (
    <div className="common-container">
      <div className="max-w-5xl w-full">
        <h2 className="h3-bold md:h2-bold text-left w-full mb-6">Research & Startup Opportunities</h2>

        {/* Tab Navigation */}
        <div className="flex gap-2 mb-6 border-b border-dark-4 overflow-x-auto">
          {(["research", "startups", "cofounders", "pitches", "myapps"] as const).map((tab) => (
            <button
              key={tab}
            ...
            >
              {tab === "myapps" ? "My Applications" : tab === "cofounders" ? "Co-Founders" : tab.charAt(0).toUpperCase() + tab.slice(1)}
            </button>
          ))}
        </div>
          ...
          <>
            {researchLoading ? (
              ...
              <div className="space-y-4">
                {research?.map((opp) => (
                  <div key={opp.id} className="p-4 rounded-lg bg-dark-3 border border-dark-4 hover:border-purple-500 transition">
                    ...
                    {opp.description && (
                      <p className="text-light-3 text-sm mb-3 line-clamp-2">{opp.description}</p>
                    )}

                    <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-sm mb-3">
                      {opp.start_date && (
                        <p className="text-light-4">📅 {new Date(opp.start_date).toLocaleDateString()}</p>
                      )}
                      {opp.duration && <p className="text-light-4">⏱️ {opp.duration}</p>}
                      {opp.stipend && <p className="text-light-4">💰 ${opp.stipend}</p>}
                      <p className="text-light-4">📊 {opp.applications_count} applied</p>
                    </div>

                    {opp.required_skills && opp.required_skills.length > 0 && (
                      <div className="mb-3">
                        <p className="text-light-2 text-xs font-semibold mb-1">Required Skills:</p>
            ...
            {!researchLoading && research?.length === 0 && (
              <div className="text-center py-12">
                <p className="text-light-3">No research opportunities available</p>
              </div>
            )}
          </>
        )}
          ...
          <>
            ...
            <div className="space-y-4">
              {startups?.map((startup) => (
                <div key={startup.id} className="p-4 rounded-lg bg-dark-3 border border-dark-4 hover:border-purple-500 transition">
                  <div className="flex justify-between items-start mb-3">
                    <div>
                      <h3 className="h4-bold text-light-1">{startup.startup_name}</h3>
                      <p className="text-light-4 text-sm">By: Founder ID {startup.creator_id}</p>
                    </div>
                  ...
                  {startup.description && (
                    <p className="text-light-3 text-sm mb-3">{startup.description}</p>
                  )}

                  {startup.looking_for && startup.looking_for.length > 0 && (
                    <div className="mb-3">
                      <p className="text-light-2 text-xs font-semibold mb-1">Looking for:</p>
                      <div className="flex gap-2 flex-wrap">
        ...
        {activeTab === "cofounders" && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {cofounders?.map((cofounder) => (
              <div key={cofounder.id} className="p-4 rounded-lg bg-dark-3 border border-dark-4 hover:border-purple-500 transition">
                <h3 className="h4-bold text-light-1 mb-2">Potential Co-Founder</h3>
                ...
                {cofounder.industry_interest && cofounder.industry_interest.length > 0 && (
                  <div className="mb-3">
                    <p className="text-light-2 text-xs font-semibold mb-1">Industries:</p>
                    <div className="flex gap-2 flex-wrap">
        ...
        {activeTab === "pitches" && (
          <div className="space-y-4">
            {competitions?.map((comp) => (
              <div key={comp.id} className="p-4 rounded-lg bg-dark-3 border border-dark-4">
                <h3 className="h4-bold text-light-1 mb-2">{comp.title}</h3>
                
                {comp.description && (
                  <p className="text-light-3 text-sm mb-3">{comp.description}</p>
                )}
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-sm mb-3">
                  ...
                  {comp.prize_pool && (
                    <p className="text-light-4">🏆 ${comp.prize_pool.toLocaleString()}</p>
                  )}
                  {comp.max_teams && (
                    <p className="text-light-4">👥 {comp.max_teams} teams max</p>
                  )}
                  <p className="text-light-4">
                    📌 By {new Date(comp.registration_deadline).toLocaleDateString()}
                  </p>
                </div>
              ...
              myResearchApps.map((app) => (
                <div key={app.id} className="p-4 rounded-lg bg-dark-3 border border-dark-4">
                  <div className="flex justify-between items-start mb-2">
                    <p className="text-light-2 font-semibold">Research Opportunity ID: {app.opportunity_id}</p>
                    <span
