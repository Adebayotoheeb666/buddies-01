...
const JobBoard = () => {
  ...
  const [activeTab, setActiveTab] = useState<
    "jobs" | "internships" | "myapplications" | "tracking"
  >("jobs");
    ...
    <div className="common-container">
      <div className="max-w-5xl w-full">
        ...
        <div className="flex gap-2 mb-6 border-b border-dark-4 overflow-x-auto">
          {(["jobs", "internships", "myapplications", "tracking"] as const).map(
            (tab) => (
              <button
                key={tab}
                ...
                {jobs?.map((job) => (
                  <div
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-sm mb-3">
                      {job.location && (
                        <p className="text-light-4">📍 {job.location}</p>
                      )}
                      {job.salary_range && (
                        <p className="text-light-4">💰 {job.salary_range}</p>
                      )}
            ...
            {internships?.map((internship) => (
              <div
                ...
                <div className="grid grid-cols-2 gap-2 text-sm mb-3">
                  <p className="text-light-4">
                    📚 GPA: {internship.gpa_requirement || "N/A"}
                  </p>
                  <p className="text-light-4">
                ...
                {internship.major_preferences &&
                  internship.major_preferences.length > 0 && (
                    <div className="mb-3">
                      <p className="text-light-2 text-xs font-semibold mb-1">
                        Preferred Majors:
                      </p>
            ...
            {applicationTracking && applicationTracking.length > 0 ? (
              applicationTracking.map((track) => (
                <div
                  ...
                  {track.notes && (
                    <p className="text-light-3 text-sm">📝 {track.notes}</p>
                  )}
                </div>
              ))
            ) : (
